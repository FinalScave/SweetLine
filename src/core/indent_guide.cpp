...
namespace NS_SWEETLINE {
  ...
  void IndentGuideAnalyzer::analyzeByScopeRules(
    ...
    for (size_t line_num = 0; line_num < line_count; ++line_num) {
      ...
      // Set nesting_level for content lines
    ...
    // Handle unclosed blocks (unclosed { at end of file)
  ...
  void IndentGuideAnalyzer::analyzeByIndentation(
    ...
    // Calculate indentation columns for each line
    ...
    // Empty line interpolation (empty line indent = min(prev non-empty, next non-empty))
    for (size_t i = 0; i < line_count; ++i) {
    ...
    // Calculate indentation level and populate line_states
    ...
    // Generate vertical guide lines - scan contiguous intervals for each indentation level
  ...
  void IndentGuideAnalyzer::analyzeByIndentationWithStart(
    ...
    // First calculate indentation columns for each line
    List<int32_t> indent_columns(line_count, 0);
    List<bool> is_empty_line(line_count, false);
    for (size_t i = 0; i < line_count; ++i) {
      const U8String& text = document->getLine(i).text;
    ...
    // Find indent-based scope rules (end == "")
    List<const ScopeRule*> indent_scope_rules = collectScopeRulesOrdered(rule, true);

    // For each line, check if there's a start token (e.g. ":")
    // If so, find the indentation rollback position from the next line as block end
    for (size_t line_num = 0; line_num < line_count; ++line_num) {
      const LineHighlight& line_highlight = highlight->lines[line_num];
      const U8String& line_text = document->getLine(line_num).text;
        ...
        auto handleIndentStartToken = [&](const U8String& token) {
          for (const ScopeRule* br : indent_scope_rules) {
            if (token == br->start) {
              ...
              // From next line, find the first non-empty line with indent <= start_indent
    ...
    // Fill in indentation level information
    for (size_t i = 0; i < line_count; ++i) {
