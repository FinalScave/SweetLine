...
std::string convertEncoding(const std::string& input,
  ...
  size_t out_bytes = in_bytes * 4; // Reserve enough space
  ...
  iconv_close(cd);
  output.resize(output.size() - out_bytes); // Resize to actual size
  return output;
}
#endif
...
namespace NS_SWEETLINE {
  ...
  U8String StrUtil::vFormatString(const char* format, va_list args) {
    va_list args_copy;
    va_copy(args_copy, args); // Copy va_list (args may be used multiple times)
    ...
    result.resize(size); // Remove trailing '\0'
    return result;
  }
