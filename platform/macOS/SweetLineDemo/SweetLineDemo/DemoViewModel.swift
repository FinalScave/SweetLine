...
final class DemoViewModel: ObservableObject {
    ...
    private func highlightFile(_ fileURL: URL, documentFileName: String) {
        ...
        do {
            ...
            status = "Warmup: \(compiledSyntaxCount) files in \(warmupElapsedMillis) ms | Analyze: \(formatMicros(analyzeMicros)) | Lines: \(lineCount(source)) | File: \(fileURL.lastPathComponent)"
        } catch {
            status = "Error: \(error)"
        }
    }
