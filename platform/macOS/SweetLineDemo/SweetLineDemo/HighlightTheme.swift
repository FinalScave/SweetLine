...
struct HighlightTheme: Identifiable, Equatable {
    ...
    var text: Color { Color(nsColor: textNSColor) }
    var backgroundNSColor: NSColor { nsColor(backgroundColor) }
    var textNSColor: NSColor { nsColor(textColor) }

    func color(for styleId: Int32) -> NSColor {
        nsColor(colorMap[styleId] ?? textColor)
    }

    func foreground(for style: TokenStyle) -> NSColor {
        switch style {
        case let .styleId(styleId):
            return color(for: styleId)
        case let .inline(inlineStyle):
            return nsColor(inlineStyle.foreground == 0 ? textColor : UInt32(bitPattern: inlineStyle.foreground))
        }
    }
    ...
    private func nsColor(_ argb: UInt32) -> NSColor {
