#if os(iOS)
import SweetLineCoreIOS
#elseif os(macOS)
import SweetLineCoreMacOS
#endif
...
public final class HighlightEngine {
    ...
    public init(config: HighlightConfig = HighlightConfig()) throws {
        guard let handle = sl_create_engine(config.showIndex, config.inlineStyle, config.tabSize) else {
    ...
    public func removeDocument(uri: String) throws {
        let handle = try nativeHandle()
        try uri.withCString { uriPtr in
            try SweetLineNative.throwIfError(
                sl_engine_remove_document(handle, uriPtr),
