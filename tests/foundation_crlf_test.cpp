...
TEST_CASE("CRLF char indexes map to positions without gaps") {
  ...
  REQUIRE(document.charIndexToPosition(4) == TextPosition{1, 1, 4});
  REQUIRE_THROWS_AS(document.charIndexToPosition(5), std::out_of_range);
}
