...
| `scopeRules` | array | 否 | 作用域规则定义（用于折叠/缩进导引线） |
...
`scopeRules` 定义作用域标记，用于编辑器的折叠功能和缩进导引线。支持两种模式：常规起止标记模式（`start` + `end`）和缩进起始标记模式（`end` 为空字符串）。
...
| `end` | string | 代码块结束标记；可为空字符串 `""` |
| `branches` | string[] | 可选，代码块内的分支关键字（如 switch 的 case） |

### 缩进起始标记模式（`end: ""`）

当 `scopeRules` 中**所有规则**的 `end` 都为 `""` 时，引擎会启用“缩进起始标记”模式：匹配到 `start` 标记后，从下一行开始根据缩进回退位置确定作用域结束。
...
**注意：**
- 不要混用 `end: ""` 与非空 `end` 规则；混用会按常规起止标记模式处理
- `start` / `end` / `branches` 按高亮 token 文本匹配，不是正则表达式
