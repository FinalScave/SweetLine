...
- [fragments - 规则片段复用](#fragments---规则片段复用)
- [states - 状态机定义](#states---状态机定义)
- [Pattern 匹配规则](#pattern-匹配规则)
- [样式系统](#样式系统)
- [状态转换](#状态转换)
- [语法导入 importSyntax](#语法导入-importsyntax)
- [子状态 subStates](#子状态-substates)
- [行尾状态 onLineEndState](#行尾状态-onlineendstate)
- [scopeRules - 作用域规则](#scoperules---作用域规则)
- [bracketRules - 括号匹配规则](#bracketrules---括号匹配规则)
...
| `fileName` | string | 否 | 精确 basename 匹配，是单个 `fileNames` 条目的简写 |
| `fileNames` | string[] | 否 | 精确 basename 列表，用于 `getSyntaxRuleByFileName()` 匹配 |
| `fileSuffix` | string | 否 | basename 后缀匹配，是单个 `fileSuffixes` 条目的简写 |
| `fileSuffixes` | string[] | 否 | basename 后缀列表，例如 `.java`、`.gradle.kts` |
| `fileNamePattern` | string | 否 | 完整 basename 正则匹配，是单个 `fileNamePatterns` 条目的简写 |
| `fileNamePatterns` | string[] | 否 | 完整 basename 正则列表，仅在精确文件名和后缀都不够表达时使用 |
| `variables` | object | 否 | 可复用的正则表达式变量定义 |
| `fragments` | object | 否 | 可复用的规则数组，可通过 `include` / `includes` 引用 |
| `styles` | array | 否 | 内联样式定义（仅 `inline_style` 模式使用） |
| `states` | object | 是 | 状态机定义，包含所有状态及其匹配规则 |
| `scopeRules` | object | 否 | 缩进导引线使用的作用域分析规则 |
| `bracketRules` | object | 否 | 彩虹括号和括号匹配分析使用的括号规则 |

---

`fileName`、`fileNames`、`fileSuffix`、`fileSuffixes`、`fileNamePattern`、`fileNamePatterns` 中至少要有一个。路由只基于 basename，默认区分大小写，匹配顺序是精确文件名优先，其次是后缀，最后才是文件名正则。
...
`fragments` 用于定义可复用的规则数组，并通过 `include` / `includes` 在 `states`（或其他 fragments）中展开，
以减少重复 JSON，同时保持规则优先级可控。
...
规则说明：
- `include`：引用单个 fragment（`string`）
- `includes`：按数组顺序引用多个 fragment（`string[]`）
- `include` / `includes` 都是在当前位置原地展开，不改变优先级
- `include` / `includes` 规则对象不能混写其他字段
- fragment 循环引用会在编译时报错
...
| `importSyntax` | string | 否* | 将其他语法规则导入当前状态 |
| `#ifdef` | string | 否 | 与 `importSyntax` 配合使用，仅在宏已定义时导入 |
| `onLineEndState` | string | 否* | 行尾自动切换状态（特殊规则，不含 pattern） |
| `include` | string | 否* | 原地展开一个已命名 fragment（特殊规则，不含 pattern） |
| `includes` | string[] | 否* | 原地展开多个已命名 fragment（特殊规则，不含 pattern） |

> `*` 注：普通匹配规则必须有 `pattern`；`onLineEndState`、`importSyntax`、`include`、`includes` 为特殊规则，不含 `pattern`。
...
`importSyntax` 用于将其他语法规则导入到当前状态。导入时会合并被导入语法的 `default` 状态规则，适合做“宿主语言 + 嵌入语言”场景（例如 Markdown 代码块）。
...
当且仅当 `HighlightEngine` 中定义了对应宏（`defineMacro("ANDROID")`）时才会导入；未定义时该导入规则会被忽略。

**注意：**
- `importSyntax` 是特殊规则，不需要 `pattern` / `style` / `styles`
- `#ifdef` 仅在 `importSyntax` 规则中生效
...
`scopeRules` 定义词法跳过区域和作用域标记，用于缩进导引线分析。它直接扫描原始文本，因此不需要先执行高亮分析。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `kind` | string | `lineComment`、`blockComment` 或 `string` |
| `start` | string | 跳过区域的起始文本 |
| `end` | string | 跳过区域的结束文本；除 `lineComment` 外必填，`string` 默认等于 `start` |
| `escape` | string | 可选，字符串类跳过规则使用的转义序列 |
| `multiLine` | boolean | 是否允许跨行；`blockComment` 固定按跨行处理 |

### Scope 规则

| 字段 | 类型 | 说明 |
|------|------|------|
| `kind` | string | `delimiter`、`word` 或 `indentStart` |
| `start` | string | 作用域开始标记 |
| `end` | string | 作用域结束标记；`delimiter` 和 `word` 必填，`indentStart` 省略 |
| `branches` | string[] | 可选，代码块内的分支关键字（如 `switch` 中的 `case`） |

`delimiter` 按字面文本匹配。`word` 还要求标记两侧是单词边界。`indentStart` 用 `:` 这类标记开启作用域，并在缩进回退时关闭作用域。
...
上例是 Python 常见写法：遇到 `:` 后，后续缩进块被识别为同一作用域。

**注意：**
- `start` / `end` / `branches` 都是字面文本标记，不是正则表达式
- 注释和字符串分隔符应放入 `skips`，这样其中的作用域标记会被忽略

---

## bracketRules - 括号匹配规则

`bracketRules` 定义用于彩虹括号渲染和括号伙伴查找的字面括号对。它直接扫描原始文本，不依赖高亮 span 或 style ID。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `pairs` | object[] | 是 | 字面括号对，数组不能为空 |
| `inheritScopeSkips` | boolean | 否 | 是否复用 `scopeRules.skips`，默认 `true` |
| `skips` | object[] | 否 | 额外跳过规则，格式与 `scopeRules.skips` 相同 |

每个 pair 都必须提供非空 `start` 和 `end` 字符串，且二者不能相同。较长的括号或 skip 标记会优先匹配，因此多字符标记可以和短标记共存。

如果某个语法没有 `scopeRules`，应直接声明 `bracketRules.skips`，让字符串和注释中的括号被忽略。如果语法已有完整的 `scopeRules.skips`，通常省略 `skips` 并使用默认的 `inheritScopeSkips: true` 即可。
