...
本文档说明 `platform/macOS/SweetLine` 中的 Swift Package SDK。

## 概览

- 模块：`SweetLine`
- 绑定方式：基于 SweetLine C API 与内置 `SweetLineCoreOSX.xcframework` 的 Swift API
- Demo 工程：`platform/macOS/SweetLineDemo`（SwiftUI/AppKit）
- 包版本：`1.2.4`

## 安装

本地开发时，在 macOS App 工程中添加此目录作为本地 Swift Package 依赖：
